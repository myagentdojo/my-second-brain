...
3. `Available portable skills`: list `hello-world`, `skill-a`, `skill-b`, `capability-tour`, `dev-mode`, `handoff-to-opus`, `new-note`, `new-project`, `runtime-custody`, and `ultragoal`, marking the first three Bun-backed and the last seven model-only.
