...
describe("handoff-to-opus model-only contract", () => {
	...
	test("keeps the main skill as a thin branch router", () => {
		...
		for (const required of [
			"For a dirty owner checkout, record the pre-edit HEAD",
			"every other owner's paths",
	...
	test("binds visible supervision, durable rollover, and fresh review", () => {
		for (const required of [
			...
			"installed `handoff` skill",
			"For **Two-axis** review, also prove it can discover",
			"dependency required by the bound mode",
			"**Single-agent** review neither requires nor invokes `code-review`",
			"Use repository-native commands and runners for every verification lane",
			"Do not use MCP runners for tests, typecheck, lint, format, or other proof",
			"stop before substituting another runner",
			...
			"Prove the resolved model and effort accept one no-work probe turn",
			"does not prove the model accepts it",
			"Treat a rejected effort as an unlaunched session",
			"rebind a supported approved effort",
			"Never let a rejected first turn consume the packet",
			"When RED needs a deliberate source perturbation",
			"require the exact revert before the next unrelated edit",
			"the GREEN rerun that proves restoration",
			"unreverted perturbation in a shared checkout is a blocker",
			"Keep shared dirty-checkout inspection read-only",
			"Use a separate clean checkout for counterfactual proof",
			"Never stash, reset, restore, checkout, clean, or switch branches",
			...
			"Bind one review delegation mode before launch",
			"admit only its declared Standards and Spec sub-agents",
			"when the user or packet forbids sub-agents",
			"do not invoke `code-review`",
